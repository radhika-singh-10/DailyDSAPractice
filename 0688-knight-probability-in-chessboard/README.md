<h2><a href="https://leetcode.com/problems/knight-probability-in-chessboard/?envType=company&envId=goldman-sachs&favoriteSlug=goldman-sachs-all">688. Knight Probability in Chessboard</a></h2><h3>Medium</h3><hr><p>On an <code>n x n</code> chessboard, a knight starts at the cell <code>(row, column)</code> and attempts to make exactly <code>k</code> moves. The rows and columns are <strong>0-indexed</strong>, so the top-left cell is <code>(0, 0)</code>, and the bottom-right cell is <code>(n - 1, n - 1)</code>.</p>
